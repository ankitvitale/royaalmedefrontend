...
function AppUserRegistration() {
    ...
    const [userType, setUserType] = useState("Supervisor"); // Display Supervisor on the frontend
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const transformedUserType = userType === "Supervisor" ? "AppUser" : userType;

        const formData = {
            name,
            email,
            password,
            user_type: transformedUserType,
            username,
        };

        try {
            const response = await axios.post(
                `${BASE_URL}/appUserRegister`,
                formData
            );
            console.log(response.data);
            if (response.data) {
                alert("AppUser Added Successfully");
            }
        } catch (error) {
            console.error(error);
        }
    ...
    useEffect(() => {
        async function getAllSupervisor() {
            try {
                const response = await axios.get(`${BASE_URL}/AllSupervisor`, {
                    headers: {
    ...
    async function handleAddSite() {
        ...
        try {
            const response = await axios.put(`${BASE_URL}/allowedSiteSupervisor/${SuperVisorid}/${residencyId}`, {}, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json"
                }
    ...
    async function handleRemoveSite() {
        ...
        try {
            const response = await axios.put(`${BASE_URL}/releaseSiteSupervisor/${removeSuperVisorId}/${residencyId}`, {}, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json"
                }
    ...
    return (
        <>
            <h2 style={{ textAlign: "center", marginTop: "50px" }}>Supervisor Registration</h2>
            ...
            <div className="app_user_registration_form_wrapper">
                <form onSubmit={handleSubmit} className="app_user_registration_form">
                    ...
                    <div>
                        ...
                        <input type="text" id="userType" value={userType} className="app_user_registration_form_input" />
                    </div>
                    ...
                    <tbody className="supervisor_table_tbody">
                        {supervisorTable.map((supervisor) => (
                            <tr key={supervisor.id}>
                                ...
                                <td>{supervisor.allowedSite.map((item, index) => {
                                    return <div key={index}>
                                        <p> {item.name}   </p>
                                    </div>
                                })} </td>
                                <td>
                                    <button onClick={() => getSuperVisorId(supervisor.id)} className="alloted_site_button">AllotSite</button>
                                    <button onClick={() => removeSuperVisor(supervisor.id)} className="remove_site_button" > Remove Site</button>
...

            {
                showSiteDropdown && (
                    <div className="selct_site_dropdown_conatiner">
                        <button onClick={() => setshowSiteDropdown(false)} className="selct_site_dropdown_conatiner_close_button"> X</button>
                        <p className="selct_site_dropdown_heading">Add Supervisor To Site</p>
                        <select value={residencyId} onChange={(e) => setresidencyId(e.target.value)} className="selct_site_dropdown_select">
                        ...
                        <button onClick={handleAddSite} className="selct_site_dropdown_submit_button"> Submit</button>
                    </div>
                )
            }
...

            {
                removesiteDropdown && (
                    <div className="selct_site_dropdown_conatiner">
                        <button onClick={() => setremovesiteDropdown(false)} className="selct_site_dropdown_conatiner_close_button"> X</button>
                        <p className="selct_site_dropdown_heading">remove Supervisor To Site</p>
                        <select value={residencyId} onChange={(e) => setresidencyId(e.target.value)} className="selct_site_dropdown_select">
                            <option value=""> Select Site</option>
                            {
                                siteName.map((item, index) => {
                        ...
                        <button onClick={handleRemoveSite} className="selct_site_dropdown_submit_button">Remove Site </button>
                    </div>
                )
            }
        </>
